...
def plot_mlt_sza_coverage(
    ...
    """
    orb_min: int = np.min(orbit)
    orb_max: int = MAX_ORBITS
    orb_res: int = 1
    orb_edg = np.arange(orb_min, orb_min + orb_max + orb_res, orb_res) - orb_res / 2

    # Generate plots encompassing all observed locations.
    for cvrg_indx, cvrg_type in enumerate(COVERAGE_TYPES):
        ...
        # science data collection status. Gather data for ALL spacecraft and MEMs first,
        # so we can standardize the scaling for all to a uniform range.
        bin_edg = np.arange(bin_min, bin_max, bin_res) - bin_res / 2
        sampled = np.zeros((len(SPACECRAFT), len(MEM_NUMBERS)))

        # Set a lower threshold for the maximum number of observations per orbit per SV
        # per day so that colorbar does not do anything weird when actual number = 0.
        cmax = 2
        ...
        for sndx, spcrft in enumerate(SPACECRAFT):
            ...
            for mem_ndx, mem_num in enumerate(MEM_NUMBERS):
                ...
                # TOD0: Do we really need to track the difference between 'use' and dce'
                # anymore, can we not just generate a single boolean mask for the points
                ...
                if np.sum(dce) > 0:
                    if cvrg_type == MLT:
                        bin_qty[mem_ndx][dce][bin_qty[mem_ndx][dce] > bin_edg[-1]] -= (
                            24.0
                        )
                    cvrg[sndx][mem_ndx], _y_edg, _x_edg = np.histogram2d(
                        bin_qty[mem_ndx][dce],
                        orbit[dce],
                        bins=(bin_edg, orb_edg),
                    )
                    cmax = max(cmax, np.nanmax(cvrg[sndx][mem_ndx]))
                else:
                    # No valid observations, create blank template.
                    cvrg[sndx][mem_ndx] = np.zeros((len(bin_edg) - 1, len(orb_edg) - 1))
                cvrg[sndx][mem_ndx][cvrg[sndx][mem_ndx] == 0] = np.nan

            logger.info(
                f"Number of samples for SV {spcrft} was {np.sum(sampled[sndx, :])}"
            )
            # end spacecraft loop

        if np.sum(sampled) == 0:
            # Don't bother to generate figures if region had 0 observations
            logger.info("Number of samples was zero, skipping.")
            continue
        ...
        for sndx, spcrft in enumerate(SPACECRAFT):
            for mem_ndx in range(NUM_MEM):
                ...
                if mem_ndx == 0:
                    ...
                    haxs.set_title(f"{spcrft}", loc="left")
...
def plot_daily_maps(
    ...
    """
    Generate polar stereographic plots showing spacecraft passes, broken down by orbit,
    in two formats:
    ...
    projections = {}
    projections[NORTH] = ccrs.NorthPolarStereo()
    projections[EQUATORIAL] = ccrs.PlateCarree()
    projections[SOUTH] = ccrs.SouthPolarStereo()
    regions = REGIONS
    ...
    for ptype in [
        ...
        fig = plt.figure(figsize=GLOBAL_FIG_SIZE)
        rows, cols = 3, 3
        mapcolspan = cols

        all_axs = np.empty((rows, cols), dtype=object)
        for sndx, spcrft in enumerate(SPACECRAFT):
            map_loc = (0, sndx)
            map_axs: GeoAxes = plt.subplot2grid(
                ...
                rowspan=1,
                colspan=1,
                projection=projections[NORTH],
            )  # ty:ignore[invalid-assignment]
            all_axs[map_loc] = map_axs

        if "geo" in ptype:
            # Map equatorial plots in geodetic coords for all SV to the same axis
            map_loc = (1, 0)
            map_axs: GeoAxes = plt.subplot2grid(
                (rows, cols),
                map_loc,
                rowspan=1,
                colspan=mapcolspan,
                projection=projections[EQUATORIAL],
            )  # ty:ignore[invalid-assignment]
            for sndx, spcrft in enumerate(SPACECRAFT):
                map_loc = (1, sndx)
                all_axs[map_loc] = map_axs
        else:
            # Map equatorial plots in magnetic coords for all SV to individual axes
            for sndx, spcrft in enumerate(SPACECRAFT):
                map_loc = (1, sndx)
                map_axs: GeoAxes = plt.subplot2grid(
                    (rows, cols),
        ...
        for regn in regions:
            ...
            lat_lower_limit = (
                GEO_LAT_LOWER_LIMIT if show_mag_lat else MAG_LAT_LOWER_LIMIT
            )
            lats_n = np.arange(
            ...
            for sndx, spcrft in enumerate(SPACECRAFT):
                ...
                map_axs = all_axs[rndx, sndx]

                if regn != EQUATORIAL:
                    proj_method = projections[regn]
                    x_0, y_0 = proj_method.transform_point(
                        0,
                        90 if regn == NORTH else -90,
                        src_crs=DATA_TRANSFORM,
                    )
                    x_1, y_1 = proj_method.transform_point(
                        45,
                        lat_lower_limit if regn == NORTH else -lat_lower_limit,
                        src_crs=DATA_TRANSFORM,
                    )
                    ...
                    # Compute a circle in axes coordinates that will be used as a
                    # clipping boundary for the map.
                    ...
                    if "geo" in ptype:
                        map_axs.set_extent(
                            (-180, +180, mlat_min, mlat_max), crs=DATA_TRANSFORM
                        )
                    else:
                        map_axs.set_extent(
                            (-60, +60, mlat_min, mlat_max), crs=DATA_TRANSFORM
                        )

                # Add features to maps in geodetic coordinates
                if "geo" in ptype:
                    map_axs.add_feature(
                        cfeature.OCEAN,
                    ...
                    lon_delta = 30.0
                    if regn == EQUATORIAL:
                        # Suppress longitudes (x/bottom), we'll add MLT ourselves
                        gl = map_axs.gridlines(
                            draw_labels={"left": "y"},
                            x_inline=False,
                            y_inline=False,
                            xlocs=np.arange(-180.0, 181.0, lon_delta),
                            ylocs=lats_n
                            if regn == NORTH
                            else [-1 * lat for lat in lats_n]
                            if regn == SOUTH
                        ...
                        # Add labels in MLT to the polar stereographic plot, with 0 at
                        # bottom (-y axis)
                        mlt_vals = np.arange(-180.0, 181.0, lon_delta)
                        mlt_desc = {}
                        for mm, mlt in enumerate(mlt_vals):
                            mlt_desc[f"{mlt:.0f}"] = f"{(mlt + 360) % 360 / 15:02.0f}H"
                        mlt_desc["-180"] = "Noon"
                        mlt_desc["-90"] = "Dusk"
                        mlt_desc["0"] = "Midnight"
                        mlt_desc["90"] = "Dawn"
                        ...
                        if (regn == NORTH) or ((regn == SOUTH) and south_inverted):
                            ccw = +1
                        else:
                            ccw = -1
                        gl = map_axs.gridlines(
                        ...
                        # Add labels in MLT to the polar stereographic plot, with 0 at
                        # bottom (-y axis)
                        mlt_vals = np.arange(0, 360.0, lon_delta)
                        mlt_desc = {}
                        for mm, mlt in enumerate(mlt_vals):
                        ...
                        mlt_desc["270"] = "Dusk"
                        for mm, mlt in enumerate(mlt_vals):
                            # Rotate to coord sys with 0 at x axis
                            mlt_pos = (ccw * mlt - 90.0) % 360.0
                ...
                if regn == NORTH:
                    map_axs.set_title(
                        f"{spcrft}", loc="center", pad=pad_mrgn, weight="bold"
                    )
                ...
                for ondx, orbnum in enumerate(unique_orbits):
                    ...
                    # TODO: Speed up plotting of maps. Why are they so slow? Is it the
                    # calculating or the rendering that is so slow? TBD
                    ...
                    # Add _second_ legend denoting MEM footprint or spacecraft activity
                    # versus position.
                    handles, labels = map_axs.get_legend_handles_labels()
                    by_label = dict(zip(labels, handles))
                    if regn == NORTH and sndx == NUM_SPC - 1:
                        map_axs.legend(
                            by_label.values(),
                            by_label.keys(),
        ...
        plt.subplots_adjust(
            ...
            bottom=0.03,
            top=0.94,
            hspace=0.12,  # irrelevant at the moment, only one row
            wspace=0.12,
        )
        save_close_figure(
            figure=fig,
